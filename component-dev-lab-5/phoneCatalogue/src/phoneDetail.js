...
class Specification extends React.Component {
  render() {
    ...
    let availability = phone
      .availability
      .map(function (avb, index) {
        return <dd key={index}>{avb}</dd>;
      });
    let dimensions = phone
      .sizeAndWeight
      .dimensions
  ...
  render() {
    var thumbImages = this
      ...
      .map(function (img, index) {
        return (
          <li>
            <img key={index} src={"/phoneSpecs/" + img} alt="missing"/>
          </li>
        );
      });
    var mainImage = (
      <div className="phone-images">
        <img
          src={"/phoneSpecs/" + this.props.phone.images[0]}
...
class PhoneDetail extends React.Component {
  ...
  componentDidMount() {
    request.get('/phoneSpecs/phones/' + this.props.params.id + '.json', (err, res) => {
